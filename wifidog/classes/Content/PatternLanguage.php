...
class PatternLanguage extends ContentGroup
{
	...
	public function displayNarrative(User $user)
	{
        ...
        $sql = "SELECT DISTINCT content_group_element_id FROM content_display_log AS cdl JOIN content_group_element AS cge ON (cdl.content_id = cge.content_group_element_id) WHERE cge.content_group_id = '{$this->getId()}' AND cdl.user_id = '{$user->getId()}' ORDER BY first_display_timestamp";
