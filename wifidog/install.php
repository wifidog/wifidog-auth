...
$exploded_path = explode("/", $_SERVER['SCRIPT_FILENAME']); # Split directories in token
array_pop($exploded_path);                                  # Remove install.php from the list
$basepath = implode("/", $exploded_path);                   # Build the basepath for this script
...
foreach($neededPackages as $key => $value) {     # Detect installed libraries (smarty, ...)
  if (file_exists($basepath . '/' . $neededPackages[$key]['file']))
...
//if(!file_exists("$basepath/$LOCAL_CONFIG_FILE"))
  $contentArray = file("$basepath/$CONFIG_FILE");
//else
//  $contentArray = file("$basepath/$LOCAL_CONFIG_FILE");
...
function saveConfig($data) {
  ...
  $contentArray = file("$basepath/$CONFIG_FILE");
  $fd = fopen("$basepath/$CONFIG_FILE", 'w');
...
#    print "<P><B>HTTPD group</B>: $process_group<BR</P>";
    ...
    foreach($dir_array as $dir) {
      ...
      $dir_info = posix_getpwuid(fileowner("$basepath/$dir"));
...
EndHTML;
    if ($neededPackages['smarty']['available']) {
      ...
      exec("pwd", $output, $return);
      print "Copying : ";
      if (!file_exists("$basepath/lib/smarty"));
  ...
  case 'dbinit':
    ...
    switch ($CONF_DBMS) {
      case 'DBMS_POSTGRES':
        ...
        $contentArray = file("$basepath/include/schema_validate.php");
        foreach($contentArray as $line) {
          #print "$line<BR>"; # Debug
        ...
        if ($result = @pg_query($connection, $sql)) { # The @ remove warning display
          ...
          else { print "Error : Unexpected result"; }
          exit();
        }
...
#          if (preg_match("/^$/", $line))  continue; # Remove empty lines
        ...
        if ($debug) print "<B STYLE=\"color:#FF0000;\">####################################################################</B>\n\n$content_mysql"; # Debug
        if ($debug) print "</PRE>";
...
EndHTML;
    ...
    foreach ($optionsInfo as $name => $foo) {         # Foreach generate all <TABLE> fields
      ...
      $selectedTrue = ''; $selectedFalse = '';        # Initialize value
