...
class ContentReport extends StatisticReport {
    ...
    */
    private function getDisplayClickThroughReport(Content $content) {
        ...
        $displayed_content_sql = "SELECT SUM(num_display) as display_count,  count(DISTINCT user_id) as display_unique_user_count, count(DISTINCT node_id) as display_unique_node_count, min(first_display_timestamp) as first_display_timestamp, max(last_display_timestamp) as last_display_timestamp FROM content_display_log WHERE content_id='$element_id'";
        $db->execSqlUniqueRes($displayed_content_sql, $display_row, false);

        $clickthrough_sql = "SELECT SUM(num_clickthrough) as clickthrough_count, count(DISTINCT user_id) as clickthrough_unique_user_count, count(DISTINCT node_id) as clickthrough_unique_node_count, min(first_clickthrough_timestamp) as first_clickthrough_timestamp, max(last_clickthrough_timestamp) as last_clickthrough_timestamp FROM content_clickthrough_log WHERE content_id='$element_id'";
    ...
    */
    private function getContentReport(Content $content) {
        ...
        $html .= "  <th>" . _("Prints") . "</th>";
        $html .= "  <th>" . _("Prints/Clickthrough") . "</th>";
        $html .= "</tr>";
        $html .= "</thead>";
        ...
        $html .= "  <td>" . sprintf("%0.2f%%", ($report_row['clickthrough_count'] / $report_row['display_count']) * 100) . "</td>";
        $html .= "</tr>\n";
        $html .= "<tr>\n";
        $html .= "  <th>" . _("First") . "</th>";
        $html .= "  <td>" . $report_row['first_clickthrough_timestamp'] . "</td>";
        $html .= "  <td>" . $report_row['first_display_timestamp'] . "</td>";
        $html .= "</tr>\n";
        $html .= "<tr>\n";
        $html .= "  <th>" . _("Last") . "</th>";
        $html .= "  <td>" . $report_row['last_clickthrough_timestamp'] . "</td>";
        $html .= "  <td>" . $report_row['last_display_timestamp'] . "</td>";
         $html .= "</tr>\n";
        ...
        $html .= "  <td>" . sprintf("%0.2f%%", ($report_row['clickthrough_unique_user_count'] / $report_row['display_unique_user_count']) * 100) . "</td>";
        $html .= "</tr>\n";
        $html .= "<tr class='odd'>\n";
        ...
        $html .= "  <td>" . sprintf("%0.2f%%", ($report_row['clickthrough_unique_node_count'] / $report_row['display_unique_node_count']) * 100) . "</td>";
        $html .= "</tr>\n";
        ...
        {
            $html .= $this->getContentGroupReport($content);
        }
        return $html;
    ...
    * @return HTML markup
    */
    private function getContentGroupReport(ContentGroup $content) {
        global $db;
        $html = null;
        ...
        foreach ($element_stats as $element_row) {
            ...
            $html .= "  <td>" . $element_row['element_display_count'] . "</td>";
