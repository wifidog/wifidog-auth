...
class File extends Content
{
	...
	// Should the admin UI display metadata about the file
	private $fileMetadataVerboseFlag = true;
	...
	private function setBinaryDataFromPostVar($upload_field)
	{
		...
		if (!empty ($_FILES[$upload_field]) && $_FILES[$upload_field]['error'] == UPLOAD_ERR_OK && $this->validateUploadedFile($_FILES[$upload_field], $output))
		{
			...
			if(empty($output)) {
				// input hasn't changed during validation
				$output = $_FILES[$upload_field];
			}

			// Updating database
			// Create a new BLOB
			$new_oid = $this->mBd->importLargeObject($output['tmp_name']);
			// Switch to new OID and touch file
			$this->setBinaryDataOid($new_oid);
			$this->setLocalFileSize($output['size']);
			$this->setMimeType($output['type']);
			$this->setFilename($output['name']);
