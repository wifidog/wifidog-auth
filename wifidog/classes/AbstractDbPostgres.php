...
class AbstractBd
{
	function connexionBd($str_baseDonnees)
	{
		if ($str_baseDonnees == NULL)
		{
			$str_baseDonnees = CONF_DATABASE_NAME;
		}

		$conn_string = "host=".CONF_DATABASE_HOST." dbname=$str_baseDonnees user=".CONF_DATABASE_USER." password=".CONF_DATABASE_PASSWORD."";
		$ptr_connexion = pg_connect($conn_string);

		if ($ptr_connexion == FALSE)
		{
			echo "Impossible de le connecter à $conn_string";
			return FALSE;
		}
	...
	*/
	function ExecuterSql($sql, & $returnResults, $debug)
	{
		$connection = $this -> connexionBd(NULL);
		if ($debug == TRUE)
		{
	...
	/**Exécute une requête pour laquelle on prévoit un résultat UNIQUE.  Si le résultat n'est pas unique, un avertissement est affiché
	 ...
	 @return TRUE si la requete a été effectuée avec succès, FALSE autrement.
	 */
	function ExecuterSqlResUnique($sql, & $retVal, $debug)
	{
		$retval = TRUE;
		if ($debug == TRUE)
		{
	 ...
	 */
	function ExecuterSqlUpdate($sql, $debug)
	{
		$connection = $this -> connexionBd(NULL);
		if ($debug == TRUE)
		{
