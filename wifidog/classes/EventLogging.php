 ...
 *
 * Logging messages of various levels, from debug to critical, from php or
 * generated by the program, can be formatted and directed to a file, be
 * displayed on the html page, as comments or as a table, or be held and
 * collected for display later in a special <div> block.
 *
 ...
 * ArrayChannel - accumulates messages in an array
 ...
 *     new ArrayChannel(new HTMLFormatter(), 'all,!debug'));
 *
 *
...
class EventLogging {
  ...
  public function flushChannels() {
    $myMessagesFlushedCounter = 0;
    if (!empty($this->stackPtrMoreSpecific))
      $myMessagesFlushedCounter += $this->stackPtrMoreSpecific->flushChannels();
  ...
  public static function stFlushChannels() {
    if (empty(self::$globalTracker)) return null;
    $beginning = self::$globalTracker;
    return $beginning->flushChannels();
  }
  ...
  public static function ExceptionHandler($pException) {
    ...
    $eo->setContext($pException->getTrace());
...
class EventFormatter {
  public function formatEvent($event) {
    ...
    $string = $this->formatMessage("log_$myErrorClass",
  ...
  public function formatMessage($pCSSClass, $pMessageType, $pMessage, $pFilename, $pLinenum) {
    $string = (!empty($pCSSClass) ? "<span class='$pCSSClass'>" : '') .
